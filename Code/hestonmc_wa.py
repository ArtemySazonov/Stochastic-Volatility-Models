...
def mc_price_wa(payoff:                 Callable,
             simulate:               Callable,
             state:                  MarketState,
...
def simulate_heston_euler_wa(state:           MarketState,
                          heston_params:   HestonParameters,
                          T:               float = 1.,
                          N_T:             int   = 100,
    ...
    for n in prange(n_simulations):
        for i in range(0,  N_T-1):
            ...
            logS[n, i+1]   = logS[n, i] + (r - 0.5 * vmax) * dt + sqrtvmaxdt * Z[0, n, i]
            V[n, i+1]      = V[n, i] + kappa*(vbar - vmax)*dt + gamma*sqrtvmaxdt*(rho*Z[0, n, i]+sqrt1_rho2*Z[1, n, i])
...
@njit(parallel=True, cache=True, nogil=True)
def simulate_heston_andersen_qe_wa(state:         MarketState,
                                heston_params: HestonParameters,
                                T:             float = 1.,
                                N_T:           int   = 100,
                                n_simulations: int   = 10_000,
    ...
    for n in prange(n_simulations):
        for i in range(N_T - 1):
            ...
            if Psi <= Psi_c:
                ...
                a           = m/(1.+b)
                b           = sqrt(b)
                V[n, i+1] = a*((b+Z[1, n, i])**2)
            else:
                p           = (Psi - 1)/(Psi + 1)
                beta        = (1.0 - p)/m
                u           = Phi(Z[1, n, i])
                V[n,i+1]  = 0. if u < p else log((1-p)/(1-u))/beta
...
def calculate_r_for_andersen_tg_wa(x_:      float,
                                maxiter: int = 2500, 
                                tol:     float = 1e-5
                                ):
    def foo(x: float):
...
@njit(parallel=True, cache=True, nogil=True)
def simulate_heston_andersen_tg_wa(state:         MarketState,
                                heston_params: HestonParameters,
                                x_grid:        np.ndarray,
